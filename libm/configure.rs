...
impl Config {
    pub fn from_env(library: Library) -> Self {
        ...
        let target_triple_split =
            target_triple.split('-').map(ToOwned::to_owned).collect();
        ...
        let cargo_features = env::vars()
            .filter_map(|(name, _value)| {
                name.strip_prefix("CARGO_FEATURE_").map(ToOwned::to_owned)
            })
        ...
        Self {
            ...
            reliable_f128: env::var_os("CARGO_CFG_TARGET_HAS_RELIABLE_F128")
                .is_some(),
            reliable_f16: env::var_os("CARGO_CFG_TARGET_HAS_RELIABLE_F16")
                .is_some(),
        }
    }
...
pub fn emit(cfg: &Config) {
    ...
    let intrinsics_enabled =
        cfg!(feature = "unstable-intrinsics") && cfg!(feature = "arch");
    ...
    let x86_no_sse2 = cfg.target_arch == "x86"
        && !cfg.target_features.iter().any(|f| f == "sse2");
