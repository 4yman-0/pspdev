    ...
    F::Int: CastFrom<u32>,
{
    ...
    let f_pow_subnorm =
        F::from_parts(false, sig_total_bits + F::EXP_BIAS, zero);
    ...
    if n > exp_max {
        ...
        if F::BITS > 16 {
            ...
            debug_assert!(
                -exp_min + F::SIG_BITS as i32 + exp_max <= add * 2 + -exp_min
            );
            ...
            let mul =
                F::from_parts(false, (F::EXP_BIAS as i32 - add) as u32, zero);
            ...
            if n < exp_min {
                let add = -(n + sig_total_bits as i32).max(exp_min);
                let mul = F::from_parts(
                    false,
                    (F::EXP_BIAS as i32 - add) as u32,
                    zero,
                );
