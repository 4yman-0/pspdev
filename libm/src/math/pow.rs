...
//
use super::{
    fabs, get_high_word, scalbn, sqrt, with_set_high_word, with_set_low_word,
};
...
pub fn pow(x: f64, y: f64) -> f64 {
    ...
    if iy > 0x41e00000 {
        ...
        let w: f64 =
            (t * t) * (0.5 - t * (0.3333333333333333333333 - t * 0.25));
...
mod tests {
    ...
    const POS_SMALL_FLOATS: &[f64] =
        &[(1.0 / 2.0), f64::MIN_POSITIVE, f64::EPSILON];
    const NEG_SMALL_FLOATS: &[f64] =
        &[-(1.0 / 2.0), -f64::MIN_POSITIVE, -f64::EPSILON];
    const POS_EVENS: &[f64] = &[2.0, 6.0, 8.0, 10.0, 22.0, 100.0, f64::MAX];
    const NEG_EVENS: &[f64] =
        &[f64::MIN, -100.0, -22.0, -10.0, -8.0, -6.0, -2.0];
    ...
    const POS: &[&[f64]] =
        &[POS_ZERO, POS_ODDS, POS_ONE, POS_FLOATS, POS_EVENS, POS_INF];
    const NEG: &[&[f64]] =
        &[NEG_ZERO, NEG_ODDS, NEG_ONE, NEG_FLOATS, NEG_EVENS, NEG_INF];
    ...
    fn test_sets_as_base(sets: &[&[f64]], exponent: f64, expected: f64) {
        sets.iter().for_each(|s| {
            s.iter().for_each(|val| pow_test(*val, exponent, expected))
        });
    }

    fn test_sets_as_exponent(base: f64, sets: &[&[f64]], expected: f64) {
        sets.iter().for_each(|s| {
            s.iter().for_each(|val| pow_test(base, *val, expected))
        });
    }
    ...
    fn infinity_as_exponent() {
        ...
        let base_below_one =
            &[POS_ZERO, NEG_ZERO, NEG_SMALL_FLOATS, POS_SMALL_FLOATS];
