...
const MIPS_NAN: bool =
    cfg!(target_arch = "mips") || cfg!(target_arch = "mips64");
    ...
    + 'static
{
    ...
    fn from_parts(
        ...
        Self::from_bits(
            (sign << (Self::BITS - 1))
                | (Self::Int::cast_from(exponent & Self::EXP_SAT)
                    << Self::SIG_BITS)
...
macro_rules! float_impl {
    (
        ...
        impl Float for $ty {
            ...
            const MIN: Self =
                $from_bits(Self::Int::MAX & !(1 << Self::SIG_BITS));
            ...
            // tests.
            const NEG_NAN: Self =
                $from_bits($to_bits(Self::NAN) | Self::SIGN_MASK);
            const NEG_SNAN: Self =
                $from_bits($to_bits(Self::SNAN) | Self::SIGN_MASK);
            ...
            fn eq_repr(self, rhs: Self) -> bool {
                ...
                fn is_nan(x: $ty) -> bool {
                    ...
                    x.to_bits() & $ty::EXP_MASK == $ty::EXP_MASK
                        && x.to_bits() & $ty::SIG_MASK != 0
                }
            ...
            fn normalize(significand: Self::Int) -> (i32, Self::Int) {
                let shift =
                    significand.leading_zeros().wrapping_sub(Self::EXP_BITS);
