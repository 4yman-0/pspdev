...
pub const fn parse_hex_exact(
    ...
    match parse_any(s, bits, sig_bits, Round::Nearest) {
        ...
        Ok((_, status)) if status.overflow() => {
            Err(HexFloatParseError("the value is too huge"))
        }
        Ok((_, status)) if status.underflow() => {
            Err(HexFloatParseError("the value is too tiny"))
        }
        Ok((_, status)) if status.inexact() => {
            Err(HexFloatParseError("the value is too precise"))
        }
        Ok(_) => unreachable!(),
    }
}
...
pub const fn parse_any(
        ...
        || bits > sig_bits + 30
    {
        return Err(HexFloatParseError(
            "unsupported target float configuration",
        ));
    }
...
mod parse_tests {
    ...
    fn rounding() {
        ...
        for k in 0..=111 {
            let (bits, status) =
                parse_any(&s, 128 - k, 112 - k, Round::Nearest).unwrap();
    ...
    fn rounding_extreme_underflow() {
        for k in 1..1000 {
            ...
            let Ok((bits, status)) = parse_any(&s, 32, 23, Round::Nearest)
            else {
                unreachable!()
            };
            ...
            assert!(
                status.inexact(),
                "should indicate inexact when parsing {s}"
            );
        }
    }
    #[test]
...
mod print_tests {
    ...
    fn spot_checks() {
        ...
        if !cfg!(x86_no_sse2) {
            ...
            assert_eq!(
                Hex(f64::NEG_SNAN).to_string(),
                "-sNaN(0x4000000000000)"
            );
            assert_eq!(
                Hex(f32::from_bits(u32::MAX)).to_string(),
                "-qNaN(0x3fffff)"
            );
            assert_eq!(
        ...
        #[cfg(f16_enabled)]
        {
            ...
            assert_eq!(
                Hex(f16::from_bits(u16::MAX)).to_string(),
                "-qNaN(0x1ff)"
            );
