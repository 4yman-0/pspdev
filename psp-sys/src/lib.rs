//! ## `psp-sys`
//!
//! Bindings to the Playstation Portable's firmware.
//! Includes panic and allocation mechanisms.
#![cfg(target_os = "psp")]
...
macro_rules! module {
    ($name:expr, $version_major:expr, $version_minor: expr) => {
        ...
        mod __psp_module {
            ...
            #[used]
            static LIB_ENT: $crate::sys::SceLibraryEntry =
                $crate::sys::SceLibraryEntry {
                    // TODO: Fix this?
                    name: core::ptr::null(),
            ...
            #[unsafe(no_mangle)]
            unsafe extern "C" fn module_start(
                argc_bytes: usize,
                argv: *mut c_void,
            ) -> isize {
                unsafe extern "C" fn main_thread(
                    argc: usize,
                    argv: *mut c_void,
                ) -> i32 {
                ...
                unsafe {
                    let id = $crate::sys::sceKernelCreateThread(
                        ...
                        $crate::sys::ThreadAttributes::USER
                            .union($crate::sys::ThreadAttributes::VFPU),
                        core::ptr::null_mut(),
                    );
...
pub fn enable_home_button() {
    ...
    unsafe {
        unsafe extern "C" fn exit_thread(
            _args: usize,
            _argp: *mut c_void,
        ) -> i32 {
            unsafe {
