unsafe extern "C" {
    ...
    ///
    /// This is not safe to call with a function that expects any other ABI.
    pub(crate) unsafe fn i_ii_i_rii(
        a: u32,
        b: u64,
        c: u32,
        ptr: unsafe extern "C" fn(u32, u64, u32) -> u64,
    ) -> u64;

    /// Call a function with the signature `fn(i32, i64, i32) -> i32` via the MIPS-EABI ABI.
    ///
    /// This is not safe to call with a function that expects any other ABI.
    pub(crate) unsafe fn i_ii_i_ri(
        a: u32,
        b: u64,
        c: u32,
        ptr: unsafe extern "C" fn(u32, u64, u32) -> u32,
    ) -> u32;
}
