...
fn update() {
    unsafe {
        //sys::sceKernelDelayThread(16000);
        init();
        clear_screen(0);

        for (i, line) in CHARS.lines().enumerate() {
            put_str::<MsxFont>(
                &line.chars[0..80], //line.len],
                0,
...
pub fn print_args(arguments: core::fmt::Arguments<'_>) {
    ...
    unsafe {
        let _ = writeln!(CHARS, "{}", arguments);
    }

    unsafe {
        let string: alloc::string::String = alloc::format!("{}\0", arguments);
        sys::sceKernelPrintf(string.as_bytes().as_ptr());
    }
...
const MSX_FONT: [u8; 2048] = *include_bytes!("../msxfont.raw");
